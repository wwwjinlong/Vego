	...
	<div id="main_table">
		<table border="1">
			...
			<tr>
				...
				<td><a href="${hostUrl}/api/code/${moduleName}/${portName}/list.do">QuickCode</a></td>
			</tr>
			<tr>
		...
		<c:if test="${remark != null}">
			<div class="beanTheme"><span class="msgType">调用说明:</span><p class="remark">&nbsp;${remark}</p></div>
		</c:if>
		...
		<table border="1">
			...
			<c:forEach var="item" items="${detaiInfo.reqList}" varStatus="status">
				<tr>
					<td>${item.name}</td>
		...
		<table border="1">
			...
			<c:forEach var="item" items="${detaiInfo.rspList}" varStatus="status">
				<tr>
					<td>${item.name}</td>
					<td>${item.type}&nbsp;${item.length}</td>
					<c:if test="${item.required == true}">
